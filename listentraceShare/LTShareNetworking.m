...
@implementation LTShareNetworking
{
    AFHTTPSessionManager *_HTTPManager;
}

+ (id)sharedInstance {
    static LTShareNetworking *request;
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        request = [[LTShareNetworking alloc] init];
    });
    return request;
}
...
+ (void)requestUrl:(NSString *)url
    ...
    [[LTShareNetworking sharedInstance] createUnloginedRequest:url WithParam:param withMethod:method success:success failure:failure showHUD:showView];
}
...
+ (void)uploadImageWithUrl:(NSString *)rul
   ...
   uploadFileProgress:(void(^)(NSProgress *uploadProgress))uploadFileProgress
              failure:(void (^)(NSError* erro))failure {
    [[LTShareNetworking sharedInstance] createUnloginedRequest:rul WithParam:param withExParam:Exparam withMethod:method success:success failure:failure uploadFileProgress:uploadFileProgress];
}
